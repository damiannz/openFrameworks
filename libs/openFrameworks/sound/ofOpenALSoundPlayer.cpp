...
set<ofOpenALSoundPlayer*>* ofOpenALSoundPlayer::players = new set<ofOpenALSoundPlayer*>;

void ofOpenALSoundPlayer::addPlayer( ofOpenALSoundPlayer* p ) {
	players->insert( p );
}
void ofOpenALSoundPlayer::removePlayer( ofOpenALSoundPlayer* p ) {
	players->erase( p );
}
...
bool ofOpenALSoundPlayer::loadSound( ofSoundBuffer sBuffer )
{
	...
	setupALSourcesAndBuffers( "ofSoundBuffer 0x"+ofToHex(&sBuffer) );
...
bool ofOpenALSoundPlayer::loadSound(string fileName, bool is_stream){
	...
	return setupALSourcesAndBuffers( fileName );
}

bool ofOpenALSoundPlayer::setupALSourcesAndBuffers( string logAppend )
{
	ALenum format;
	if ( channels == 1 )
		format = AL_FORMAT_MONO16;
	...
	if(channels==1){
		...
		if (err != AL_NO_ERROR)
		{
			ofLogError("ofOpenALSoundPlayer") << "error " << err << " generating source for " << logAppend;
			return false;
		}
		...
		for(int i=0; i<(int)buffers.size(); i++){
			...
			err = alGetError();
			if (err != AL_NO_ERROR){
				ofLogError("ofOpenALSoundPlayer:") << "error " << err << " creating buffer for " << logAppend;
				return false;
			}
			if(isStreaming){
		...
		err = alGetError();
		if (err != AL_NO_ERROR){
			ofLogError("ofOpenALSoundPlayer") << "error " << err << " creating stereo sources for " + logAppend;
			return false;
		}
		...
		if(isStreaming){
			for(int s=0; s<2;s++){
				for(int i=0;i<channels;i++){
					...
					err = alGetError();
					if ( err != AL_NO_ERROR){
						ofLogError("ofOpenALSoundPlayer") << "error " << err << " creating stereo buffers for " + logAppend;
						return false;
					}
...
//
void ofOpenALSoundPlayer::setAutoUpdate(bool tf)
{
	if ( tf && !addedAsListener )
	{
		if ( addedAsListener )
			ofAddListener(ofEvents().update,this,&ofOpenALSoundPlayer::update);
		addedAsListener = true;
	}
	else if ( !tf && addedAsListener )
	{
		if ( addedAsListener )
